...
export function AdminSidebar({
  ...
  return (
    <aside className="flex h-screen w-56 flex-col bg-brand-dark text-brand-paper">
      {/* Logo + mobile close */}
      <div className="flex items-center border-b border-brand-paper/10 px-4 py-3">
        <Logo height={34} variant="dark-bg" />
        <div className="ml-auto flex items-center gap-1.5">
          <span className="font-mono text-[9px] uppercase tracking-wider text-brand-paper/40">
            Admin
          </span>
          {onClose && (
            <button
              onClick={onClose}
              className="ml-1.5 rounded p-0.5 text-brand-paper/50 hover:text-brand-paper lg:hidden"
            ...
            return (
              <li key={item.href}>
                ...
                >
                  ...
                  {item.label}
                  {item.badge ? (
                    <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-brand-amber px-1 font-mono text-[10px] font-bold text-brand-dark">
